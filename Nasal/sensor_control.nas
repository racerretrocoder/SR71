...
#sensor gui dialog
var sensors_dialog = gui.Dialog.new("sensors_dialog/dialog","Aircraft/SR71/Dialogs/sensor_interface.xml");
...
var sensor_loop = func() {
	...
	#and math.abs(getprop("/orientation/pitch-deg")) < 5 and math.abs(getprop("/orientation/roll-deg")) < 5 )
...
var capture_tgt = func(mp, my_pos, radar_heading) {
	if ( radar_logic.isNotBehindTerrain(mp) == 1 ) { # can be seen, is good
		...
		screen.log.write(target_callsign);
		screen.log.write("SLR Captured");
		if ( getprop(mode) == slr ) {
...
#start recording
timer_rec = maketimer(2, sensor_loop);
timer_log = maketimer(1, output_log);
    timer_log.start();
	timer_rec.start();
#setlistener("/controls/sensors/recording", func { } );
