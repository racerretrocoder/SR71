...
var cdlg_traj_map = {
	...
	init: func {
		...
		me.window.del = func()
			{
		  	canvasdl.cdlg_traj_map.update_loop_flag = 0;
			removelistener(canvasdl.cdlg_traj_map.listener1);
			removelistener(canvasdl.cdlg_traj_map.listener2);
		...
		me.button_prediction.f = func {
			...
			if (me.index == 0)
				{canvasdl.cdlg_traj_map.num_points_prediction = 0;}
			else if (me.index == 1)
				{canvasdl.cdlg_traj_map.num_points_prediction = 36;}
			else if (me.index == 2)
				{canvasdl.cdlg_traj_map.num_points_prediction = 64;}
			else if (me.index == 3)
				{canvasdl.cdlg_traj_map.num_points_prediction = 72;}
			else if (me.index == 4)
				{canvasdl.cdlg_traj_map.num_points_prediction = 108;}

			canvasdl.cdlg_traj_map.trajectory_is_drawn = 0;
		};
		...
		me.button_horizon.f = func {
			...
			if (me.index == 0)
				{canvasdl.cdlg_traj_map.num_points_horizon = 20;}
			else if (me.index == 1)
				{canvasdl.cdlg_traj_map.num_points_horizon = 0;}

			canvasdl.cdlg_traj_map.horizon_is_drawn = 0;
		};
		...
		me.button_comsites.f = func {
			...
			canvasdl.cdlg_traj_map.com_site_flag = me.index;
			canvasdl.cdlg_traj_map.comsites_are_drawn = 0;
		};
		...
		me.button_landsites.f = func {
			...
			canvasdl.cdlg_traj_map.land_site_flag = me.index;
			canvasdl.cdlg_traj_map.land_sites_are_drawn = 0;
		};
		...
		me.button_history.f = func {
			...
			canvasdl.cdlg_traj_map.history_draw_flag = me.index;
			canvasdl.cdlg_traj_map.history_is_drawn = 0;
		};
	...
	update: func {
		...
		me.MET = getprop("/sim/time/elapsed-sec") + 0; # delta-MET
...
#
	...
	update_landing_site: func {
		...
		var x = me.lon_to_x(0);    # SpaceShuttle.landing_site.lon()
		var y = me.lat_to_y(0);    # SpaceShuttle.landing_site.lat()
	...
	task_alphanumerics: func {
		...
		me.an_met.setText("MET: "~me.MET);
		me.an_landing_site.setText("Landing site: NONE"); # getprop("/sim/gui/dialogs/SpaceShuttle/entry_guidance/site")
